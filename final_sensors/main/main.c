...
void app_main()
{
    ...
    xTaskCreate(ds3231, "time", configMINIMAL_STACK_SIZE * 5, NULL, 5, NULL);
...
void ds3231(void *pvParameters)
{
    ...
    time(&now);
    localtime_r(&now, &timeinfo);
    ESP_LOGI(TAG, "time before setting on the ds: %02d-%02d-%04d  %02d:%02d:%02d", timeinfo.tm_mday, timeinfo.tm_mon, timeinfo.tm_year + 1900, timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
    if (ds3231_set_time(&dev, &timeinfo)==ESP_OK){
        ESP_LOGI(TAG, "time set on ds");
    }
